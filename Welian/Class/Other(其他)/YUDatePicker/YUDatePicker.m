...
#define XYNUMBEROFROWS 16384
#define XYROSE_NUMBER 99
...
-(NSDate *)date
{
    NSString *ds = [NSString stringWithFormat:XY_FORMATSTR,
    ...
    NSDate *date = [NSDate dateFromString:ds withFormat:XY_FORMAT];
    return date;
    
}

-(NSString *)dateStr{
    NSString *ds = [NSString stringWithFormat:XY_DATESTR,
                    self.yearArray[self.yearIndex],
                    self.monthArray[self.monthIndex],
                    self.dayArray[self.dayIndex],
                    self.hourArray[self.hourIndex],
...
{
    if (!minimumDate) {
        _minimumDate = [NSDate dateWithYea:XYPICKER_MINDATE];
    }else{
...
{
    if (!_maximumDate) {
        _maximumDate = [NSDate dateWithYea:XYPICKER_MAXDATE];
    }else{
...
-(void)setCurrentDate:(NSDate *)currentDate{
    if (currentDate) {
        ...
        self.yearIndex = d.year-XYPICKER_MINDATE;
...
-(void)initData{
    ...
    for (int i=0; i<XYPICKER_MINUTE; i++) {
        NSString *num = [NSString stringWithFormat:@"%02d",i];
        if (0<i && i<=XYPICKER_MONTH)
            [self.monthArray addObject:num];
        if (0<i && i<=XYPICKER_DAY)
...
//        UIBarButtonItem *cancelButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(actionCancel:)];
        ...
        [cancelBtn sizeToFit];
        cancelBtn.width = cancelBtn.width + 8.f;
...
//        UIBarButtonItem *doneBtn = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(actionDone:)];
        ...
        [doneBtn sizeToFit];
        doneBtn.width = doneBtn.width + 8.f;
    ...
    switch (self.datePickerMode) {
        ...
        case UIYUDatePickerModeDateYYYYMMDDHHmm:{
            ...
            if (component == 1) {
                ...
                [pickerView selectRow:self.dayIndex + self.dayArray.count * XYROSE_NUMBER inComponent:2 animated:NO];
            }
            if (component == 2) {
...
-(void)revertRow:(NSDate*)date animated:(BOOL)animated{
    ...
    for (int i=0; i<arry.count; i++) {
        if (i%2) {
            ...
            row = [arry[i]integerValue]*XYROSE_NUMBER;
        }
    }
}
...
{
    if (_doneSelectDateBlock) {
        _doneSelectDateBlock(NO,self.dateStr);
    }
    [self hidden];
}
...
{
    if (_doneSelectDateBlock) {
        _doneSelectDateBlock(YES,self.dateStr);
    }
    [self hidden];
}
